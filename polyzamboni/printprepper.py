...
class ComponentPrintData():
    ...
    def __init__(self):
        self.bb = (np.zeros(2), np.zeros(2))
        self.cut_edges = []
        self.concave_fold_edges = []
        self.convex_fold_edges = []
    ...
    def add_concave_fold_edge(self, concave_edge):
        self.concave_fold_edges.append(concave_edge)
    
    def add_convex_fold_edge(self, convex_edge):
        self.convex_fold_edges.append(convex_edge)

    def add_glue_flap_edge(self, glue_flap_edge):
...
def component_fits_in_page_part_node(component : ComponentPrintData, node : PagePartitionNode):
    comp_width = component.bb[1][0] - component.bb[0][0]
    comp_height = component.bb[1][1] - component.bb[0][1]
...
def recursively_collect_all_page_components(node : PagePartitionNode, component_list, target_list):
    ...
    if node.contained_component_id is not None:
        ...
        translation =  AffineTransform2D(affine_part = node.lower_left_corner - component_in_node.bb[0])
        target_list.append({"print data" : component_in_node, "page coord transform" : translation})
    recursively_collect_all_page_components(node, component_list, target_list)
    recursively_collect_all_page_components(node, component_list, target_list)
...
def fit_components_on_pages(components, page_size, page_margin, component_margin, different_materials_on_different_pages):
    ...
    sorted_components = sorted(components, key = lambda c : (c.bb[1][0] - c.bb[0][0]) * (c.bb[1][1] - c.bb[0][1]))
    ...
    for current_component_id, component_print_data in enumerate(sorted_components):
        ...
        if different_materials_on_different_pages and component_mat_id not in page_partitions.keys():
            # create first page for this material
            new_page = create_new_page_partition(page_size, page_margin)
            page_partitions[component_mat_id] = []
            page_partitions[component_mat_id].append(new_page)
            # add component to the created page
            if not component_fits_in_page_part_node(component_print_data, new_page):
                print("POLYZAMBONI WARNING: Unfolded connected component does not fit on one paper! ")
        ...
        node_candidates = []
        for mat_id, page_list in page_partitions.ites():
        ...
        if len(node_candidates) == 0:
            ...
            partition_node_after_component_insertion(component_print_data, smallest_viable_node)

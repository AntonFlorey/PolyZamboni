"""
This file contains papermodel classes. It is the core of this addon.
"""
...
class ConnectedComponent():
    ...
    @classmethod
    def from_mesh_props(cls, face_indices, cyclic, overlapping, unfolding_affine_transforms, unfolded_face_geometry, glueflap_geometry, glueflap_collisions, build_step_number, render_data_outdated):
...
class PaperModel():
    ...
    def new_from_mesh(cls, mesh : Mesh):
        ...
        papermodel_instance.newly_created = True
        return papermodel_instance
    ...
    def __exit__(self, exc_type, exc_value, exc_tb):
        if exc_type is None:
            # only close paper model and write back data if no exception occured while editing
            self.close()
        else:
            print("Something went wrong while working on the paper model! Changes are not safed.")
            self.bm.free()
    ...
    def close(self):
        ...
        if self.valid:
            ...
            if self.newly_created:
                io.write_all_component_render_data(self.mesh, {}, {}) # write empty render data property
    ...
    def clear_edges(self, indices_of_edges_to_clear):
        pass
    ...
    def __component_tree_search(self, start_face_index, visited_faces : set):
    ...
    def __recompute_all_connected_components(self):
        ...
        for face_index in self.dual_graph.nodes:
            if face_index in visited_faces:
                continue
            tree_traversal, pred_dict, component_has_cycles = self.__component_tree_search(face_index, visited_faces)
    ...
    def __recompute_connected_components_around_edges(self, edge_index_list):
            ...
            while dfs_stack:
                ...
                if not curr_e.is_boundary and (connected_component_01.cyclic or connected_component_10.cyclic):
                    ...
                    if curr_e.index not in self.glueflap_dict.keys():
                        ...
                        used_halfedge, no_new_overlap = self.__add_glueflap_to_one_halfedge(preferred_halfedge)
                        curr_flap_01 = used_halfedge == (v0.index, v1.index)
                        if no_flap_overlaps and not no_new_overlap:
                        ...
                        if component_index_with_flap in updated_components:
                            ...
                            if no_flap_overlaps and collision_occured:
                                no_flap_overlaps = False
